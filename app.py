...
OUTPUT_DIR = "tree_tags_balanced_en" 
...
def generate_tags(tree_data):
    ...
    for tree in tree_data:
        ...
        draw.text(((WIDTH_PX - title_w) / 2, text_y), tree["title"], font=font_title, fill='black')
        ...
        draw.text(((WIDTH_PX - sub_w) / 2, text_y), tree["subtitle"], font=font_subtitle, fill='black')
        ...
        # 生成初始黑白图像
        qr_img = qr.make_image(fill_color="black", back_color="white").get_image().convert("1", dither=Image.NONE)
        
        # 强制缩放：设定二维码的绝对物理大小 (占据卡片宽度的 75%，约 478 像素)
        ...
        qr_y = HEIGHT_PX - qr_h - 90 # 距离底部固定 70px
